...
-- Hardcoded Aura list
local AuraList = {
	"Scarlet Ruin", "Absolute Ego", "Ultra Instinct", "Divine Radience",
	"Phantom Haze", "Shadow Chaos", "Cyro Storm", "Luminous Core",
	"Crimson Surge", "Void Heart", "Cosmic Collapse", "Inferno Pulse", "Cursed King",
}
...
local function GetWorld()
	...
	local success, result = pcall(function()
		return Player:FindFirstChild("World")
	end)
	if success and result and result:IsA("ValueBase") then
		return result.Value
	end
...
local function DoRebirth()
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("WallhopRemotes"):WaitForChild("RebirthRequest")
		Event:FireServer()
	end)
end
...
local function BuyAura(AuraName)
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("WallhopRemotes"):WaitForChild("AuraBuy")
...
local Window = Library:CreateWindow({
	...
	Footer = {
		{ Text = "discord.gg/jdJvZm6VdK", Copyable = true },
		{ Text = " | " },
...
UpdatesBox:AddLabel({ Text = '<font color="#8a8a8a"> Last Updated 8/24/2026</font>' })
...
AddFeatureToggle(FarmGroup, "AutoBuyAura", {
	Text = "Auto Buy Aura",
...
MenuBox:AddButton({ Text = "Stop All Features", Func = function()
	Config.FarmWinsActive = false Config.RebirthActive = false Config.BuyAuraActive = false
