...
local Config = {
	...
	AutoBuyRock = false,
	AutoUpgrade = false,
	AutoBuyUpgradeA = false,
	AutoBuyUpgradeB = false,
	AutoClaimPotion = false,
	AutoSave = false,
	AutoExecute = false,
	AutoReconnect = false,
	AutoHideUi = false,
...
-- Cache ALL potion drop parts from PotionDrops folder
local PotionDropParts = {}
local function CacheAllPotionDrops()
	PotionDropParts = {}
	pcall(function()
		local potionFolder = Workspace:WaitForChild("PotionDrops", 5)
		if potionFolder then
			for _, child in ipairs(potionFolder:GetChildren()) do
				local defaultPart = child:FindFirstChild("default")
				if defaultPart and defaultPart:IsA("BasePart") then
					table.insert(PotionDropParts, defaultPart)
				elseif child:IsA("BasePart") then
					table.insert(PotionDropParts, child)
				else
					for _, sub in ipairs(child:GetChildren()) do
						if sub:IsA("BasePart") then
							table.insert(PotionDropParts, sub)
							break
						end
					end
				end
...
task.spawn(CacheAllPotionDrops)
task.spawn(function()
	while true do
		task.wait(15)
		CacheAllPotionDrops()
...
local function GetRockList()
	...
	pcall(function()
		...
		if MassTiers:IsA("ModuleScript") then
			local success, data = pcall(require, MassTiers)
			if success and type(data) == "table" then
				local list = data.List
				if type(list) == "table" then
					for _, item in ipairs(list) do
						if type(item) == "table" and item.Id then
							table.insert(RockList, { Id = item.Id, Name = item.Name or tostring(item.Id) })
						end
					end
				end
				if #RockList == 0 then
		...
		elseif MassTiers:IsA("Folder") then
			for _, child in ipairs(MassTiers:GetChildren()) do
				...
				table.insert(RockList, { Id = idAttr or child.Name, Name = child.Name })
			end
		end
	end)
	return RockList
...
-- 1. AUTO THROW FAST (Throw first, wait 1s, then RevealDone)
local function AutoThrowFast()
	pcall(function()
		local ThrowEvent = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("Throw")
		ThrowEvent:InvokeServer()
	end)
	task.wait(1)
	pcall(function()
		local RevealEvent = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("RevealDone")
		RevealEvent:FireServer()
	end)
end

-- 1.5 INSTANT UNLOCK W2
local function AutoUnlockW2()
	pcall(function()
		local TravelEvent = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("TravelTo")
		TravelEvent:InvokeServer(2)
	end)
end
...
local function AutoSellLoop()
	pcall(function()
		local SellEvent = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("SellAll")
		SellEvent:FireServer()
	end)
end

-- 3. AUTO BUY MASS (read MassTiers.List, buy all by numeric Id)
local function AutoBuyRock()
	local rockList = GetRockList()
	if #rockList == 0 then return end
	for _, rock in ipairs(rockList) do
		pcall(function()
			local BuyEvent = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("BuyMass")
			BuyEvent:InvokeServer(rock.Id)
		end)
		task.wait(0.1)
	end
end

-- 4. AUTO UPGRADE (Gravity + Density)
local function AutoUpgrade()
	pcall(function()
		local BuyUpgrade = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("BuyUpgrade")
		BuyUpgrade:InvokeServer("Gravity", "max")
	end)
	pcall(function()
		local BuyUpgrade = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("BuyUpgrade")
		BuyUpgrade:InvokeServer("Density", "max")
	end)
end

-- 5. AUTO CLAIM POTION (teleport, claim, return)
local function AutoClaimPotion()
	if not HumanoidRootPart or not HumanoidRootPart.Parent then return end
	if #PotionDropParts == 0 then
		CacheAllPotionDrops()
	end
	...
	for _, dropPart in ipairs(PotionDropParts) do
		pcall(function()
			if dropPart and dropPart.Parent then
				HumanoidRootPart.CFrame = dropPart.CFrame + Vector3.new(0, 3, 0)
				task.wait(0.2)
				HumanoidRootPart.CFrame = dropPart.CFrame + Vector3.new(0, 0.5, 0)
				task.wait(0.3)
			end
		end)
	end
...
local function RunAutoExecute()
	task.delay(3, function()
		if Config.AutoExecute then
			local Toggle = Library.Toggles.AutoThrowFast
...
-- CONFIG SAVE / LOAD
local ConfigsDir = "RockThrower/Configs"
local AutoloadPath = "RockThrower/Autoload.json"
...
task.spawn(function()
	...
	if Success and Info and Info.Name then
		pcall(function()
			GameNameLabel:SetText('GAME - <font color="#60d888">' .. Info.Name .. '</font>')
		end)
	else
		pcall(function()
...
FeaturesBox:AddLabel({ Text = '<font color="#60d888">✓ Auto Throw Rock</font>' })
FeaturesBox:AddLabel({ Text = '<font color="#60d888">✓ Instant Unlock W2</font>' })
FeaturesBox:AddLabel({ Text = '<font color="#60d888">✓ Auto Sell All</font>' })
FeaturesBox:AddLabel({ Text = '<font color="#60d888">✓ Auto Buy Rock</font>' })
FeaturesBox:AddLabel({ Text = '<font color="#60d888">✓ Auto Upgrade</font>' })
FeaturesBox:AddLabel({ Text = '<font color="#60d888">✓ Auto Claim Potion</font>' })
...
-- ===== MAIN TAB - FARMING GROUP (LEFT) =====

local FarmBox = Tabs.Main:AddLeftGroupbox("Auto Farming", "star")

AddFeatureToggle(FarmBox, "AutoThrowFast", {
	Text = "Auto Throw Rock",
	Tooltip = "Auto Throw",
	Notify = true,
}, function(Value)
	Config.AutoThrowFast = Value
	if Value then
		task.spawn(function()
			while Config.AutoThrowFast do
				AutoThrowFast()
				task.wait(0.00001)
			end
		end)
...
AddFeatureToggle(FarmBox, "AutoBuyRock", {
	Text = "Auto Buy Rock",
	Tooltip = "Auto Buy Rock",
	Notify = true,
}, function(Value)
	Config.AutoBuyRock = Value
	if Value then
		task.spawn(function()
			while Config.AutoBuyRock do
				AutoBuyRock()
				task.wait(0.5)
			end
		end)
...
AddFeatureToggle(FarmBox, "AutoClaimPotion", {
	Text = "Auto Claim Potion",
	Tooltip = "Auto Claim Potion",
	Notify = true,
}, function(Value)
	Config.AutoClaimPotion = Value
	if Value then
		task.spawn(function()
			while Config.AutoClaimPotion do
				AutoClaimPotion()
				task.wait(0.5)
			end
		end)
...
AddFeatureToggle(FarmBox, "AutoSell", {
	Text = "Auto Sell All",
...
FarmBox:AddButton({
	...
	Func = function()
		AutoUnlockW2()
		Notify("W2", "World 2 unlocked!", "Success")
	end,
	Tooltip = "Instantly unlock World 2"
})

-- ===== UPGRADE GROUP (RIGHT) =====

local UpgradeBox = Tabs.Main:AddRightGroupbox("Upgrade", "trending-up")

AddFeatureToggle(UpgradeBox, "AutoUpgrade", {
	Text = "Auto Upgrade All",
	Tooltip = "Auto Upgrade All",
	Notify = true,
}, function(Value)
...
AddFeatureToggle(UpgradeBox, "AutoBuyUpgradeA", {
	Text = "Auto Upgrade Luck",
	Tooltip = "Auto Upgrade Luck",
	Notify = true,
}, function(Value)
	Config.AutoBuyUpgradeA = Value
	if Value then
		task.spawn(function()
			while Config.AutoBuyUpgradeA do
				pcall(function()
					local BuyUpgrade = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("BuyUpgrade")
					BuyUpgrade:InvokeServer("Gravity", "max")
				end)
...
AddFeatureToggle(UpgradeBox, "AutoBuyUpgradeB", {
	Text = "Auto Upgrade Value",
	Tooltip = "Auto Upgrade Value",
	Notify = true,
}, function(Value)
	Config.AutoBuyUpgradeB = Value
	if Value then
		task.spawn(function()
			while Config.AutoBuyUpgradeB do
				pcall(function()
					local BuyUpgrade = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("BuyUpgrade")
					BuyUpgrade:InvokeServer("Density", "max")
				end)
...
MenuBox:AddButton({
	...
	Func = function()
		Config.AutoThrowFast = false
		Config.AutoUnlockW2 = false
		Config.AutoSell = false
		Config.AutoBuyRock = false
		Config.AutoUpgrade = false
		Config.AutoBuyUpgradeA = false
		Config.AutoBuyUpgradeB = false
		Config.AutoClaimPotion = false
