...
-- Hardcoded Aura & Trail lists
local AuraList = {
	"Lightning", "Fire", "Cosmic Core", "Blessed", "Astral",
	"Hell Halo", "Flame Ego", "Devil Ego", "Cursed", "Void Pulse", "Neutron Collapse",
}

local TrailList = {
	"Red", "Blue", "Green", "Purple", "Black", "Gold",
	"Molten Lava", "Diamond", "Rainbow", "Void", "Polar", "Solar",
}
...
local function GetWinPad()
	local success, result = pcall(function()
		return Workspace.Map.WinPads.DefaultWins.Zone55
	end)
...
local function InstantUnlockW4()
	pcall(function()
		if HumanoidRootPart and HumanoidRootPart.Parent then
			HumanoidRootPart.CFrame = CFrame.new(-968.708435, 17.4787216, 1755.98254)
			RunService.RenderStepped:Wait(1)
		end
		local Event = ReplicatedStorage:WaitForChild("TreeGame"):WaitForChild("Remotes"):WaitForChild("WorldTeleport")
		Event:FireServer(4)
	end)
end
...
local function FarmLevel()
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("TreeGame"):WaitForChild("Remotes"):WaitForChild("ClickHeat")
		Event:FireServer()
	end)
end
...
local function DoRebirth()
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("TreeGame"):WaitForChild("Remotes"):WaitForChild("RebirthRequest")
		Event:FireServer()
	end)
end
...
local function BuyAura(AuraName)
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("TreeGame"):WaitForChild("Remotes"):WaitForChild("AuraBuy")
		Event:FireServer(AuraName)
	end)
end

local function BuyAllAuras()
...
local function BuyTrail(TrailName)
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("TreeGame"):WaitForChild("Remotes"):WaitForChild("TrailBuy")
...
FeaturesBox:AddLabel({ Text = '<font color="#60d888">Instant Unlock W4</font>' })
...
FeaturesBox:AddLabel({ Text = '<font color="#60d888">Auto Buy All Auras</font>' })
...
AddFeatureToggle(FarmGroup, "AutoFarmWins", {
	Text = "Auto Farm Wins",
	Tooltip = "Touch workspace.Map.WinPads.DefaultWins.Zone55",
	Notify = true,
}, function(Value)
...
FarmGroup:AddButton({
	Text = "Instant Unlock W4",
	Func = function()
		InstantUnlockW4()
		Notify("W4", "Teleporting and unlocking World 4...", "Success")
	end,
	Tooltip = "Teleport to W4 and unlock it",
})

-- SHOP GROUP
local ShopGroup = FarmingTab:AddRightGroupbox("Shop", "shopping-cart")

AddFeatureToggle(ShopGroup, "AutoBuyAura", {
	Text = "Auto Buy Aura",
	Tooltip = "Buy all auras in list",
	Notify = true,
}, function(Value)
	Config.BuyAuraActive = Value
	if Value then
		task.spawn(function()
			while Config.BuyAuraActive do
				BuyAllAuras()
				task.wait(0.3)
			end
		end)
...
AddFeatureToggle(ShopGroup, "AutoBuyTrail", {
	Text = "Auto Buy Trail",
	Tooltip = "Buy all trails in list",
	Notify = true,
}, function(Value)
	Config.BuyTrailActive = Value
	if Value then
		task.spawn(function()
			while Config.BuyTrailActive do
				BuyAllTrails()
				task.wait(0.3)
			end
		end)
...
MenuBox:AddButton({ Text = "Stop All Features", Func = function()
	Config.FarmWinsActive = false Config.FarmLevelActive = false Config.RebirthActive = false Config.BuyAuraActive = false Config.BuyTrailActive = false
