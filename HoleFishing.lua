...
end, Tooltip = "FULL INDEX" })

AddFeatureToggle(FarmBox, "AutoFishSecret", { Text = "Auto Fish Secret [HUGE]", Tooltip = "Auto Fish Secret", Notify = true }, function(v)
	Config.AutoFishSecret = v
	if v then task.spawn(function() while Config.AutoFishSecret do AutoFishSecret() task.wait(1) end end) end
end)

AddFeatureToggle(FarmBox, "AutoSell", { Text = "Auto Sell", Tooltip = "Sell inventory automatically", Notify = true }, function(v)
	Config.AutoSell = v
	if v then task.spawn(function() while Config.AutoSell do AutoSell() task.wait(0.5) end end) end
end)
