...
-- ============================================================
-- GAME-SPECIFIC: Remotes, Positions, Data
-- ============================================================

-- Win position CFrame (+4 studs higher on Y)
local WinCFrame = CFrame.new(-5.48531342, 5567.75, 794.886475, 0, 0, -1, 0, 1, 0, 1, 0, 0)

-- Remote Events
local RebirthRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("RebirthRequest")
local AuraRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("AuraRequest")
local EggEvent = ReplicatedStorage:WaitForChild("PetSystem"):WaitForChild("Events"):WaitForChild("OpenEgg")
local InventoryRemote = ReplicatedStorage:WaitForChild("PetSystem"):WaitForChild("Events"):WaitForChild("InventoryRemote")
...
local Config = {
	AutoFarmWinsActive = false,
	AutoRebirthActive = false,
	AutoRollAuraActive = false,
	AutoBuyEggActive = false,
	AutoEquipPetActive = false,
...
local function DoAutoFarmWins()
	...
	pcall(function()
		...
		local WinPad = Workspace:FindFirstChild("Map")
			and Workspace.Map:FindFirstChild("WinPads")
			and Workspace.Map.WinPads:FindFirstChild("Win pad21")
			and Workspace.Map.WinPads["Win pad21"]:FindFirstChild("Part")
...
-- AUTO ROLL AURA [OP]
local function DoAutoRollAura()
	pcall(function()
		AuraRemote:InvokeServer("Roll", nil)
	end)
end

-- AUTO BUY BEST EGG
local function DoAutoBuyEgg()
	pcall(function()
		EggEvent:FireServer({
			Action = "Hatch",
			EggName = "JellyBox",
...
-- AUTO EQUIP BEST PET
local function DoAutoEquipBestPet()
	pcall(function()
...
local function RunAutoExecute()
	task.delay(3, function()
		if Config.AutoExecute then
			local Toggle = Library.Toggles.AutoFarmWins
...
UpdatesBox:AddLabel({ Text = '<font color="#8a8a8a"> Last Updated 8/20/2026</font>' })
...
FeaturesBox:AddLabel({ Text = '<font color="#60d888">Auto Roll Aura [OP]</font>' })
FeaturesBox:AddLabel({ Text = '<font color="#60d888">Auto Buy Best Egg</font>' })
...
AddFeatureToggle(FarmBox, "AutoRollAura", {
	Text = "Auto Roll Aura [OP]",
	Tooltip = "Auto Roll Aura [OP]",
	Notify = true,
}, function(Value)
	Config.AutoRollAuraActive = Value
	if Value then
		task.spawn(function()
			while Config.AutoRollAuraActive do
				DoAutoRollAura()
...
-- RIGHT GROUP: Egg
local EggBox = MainTabs.AutoFarm:AddRightGroupbox("Egg", "egg")
...
AddFeatureToggle(EggBox, "AutoBuyEgg", {
	...
	if Value then
		task.spawn(function()
			while Config.AutoBuyEggActive do
				DoAutoBuyEgg()
...
AddFeatureToggle(EggBox, "AutoEquipPet", {
	Text = "Auto Equip Best Pet",
	Tooltip = "Auto Equip Best Pet",
	Notify = true,
}, function(Value)
	Config.AutoEquipPetActive = Value
	if Value then
		task.spawn(function()
			while Config.AutoEquipPetActive do
				DoAutoEquipBestPet()
				RunService.RenderStepped:Wait(1.3)
			end
		end)
	end
end)
...
MenuBox:AddButton({
	...
	Func = function()
		Config.AutoFarmWinsActive = false
		Config.AutoRebirthActive = false
		Config.AutoRollAuraActive = false
		Config.AutoBuyEggActive = false
		Config.AutoEquipPetActive = false
