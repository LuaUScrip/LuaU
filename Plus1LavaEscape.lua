...
-- Upgrade list
local UpgradeList = {"BurnTime", "Cooldown", "Diameter"}

-- Win pad part (cached)
local WinPadPart = nil
local function CacheWinPad()
	pcall(function()
		WinPadPart = Workspace:WaitForChild("Wins"):WaitForChild("BedrockBlock"):WaitForChild("Wins"):WaitForChild("Main", 5)
	end)
end
task.spawn(CacheWinPad)
task.spawn(function()
	while true do
		task.wait(10)
		if not WinPadPart or not WinPadPart.Parent then
			CacheWinPad()
		end
	end
end)

-- Dynamically read aura list from ReplicatedStorage.N1.AuraConfig
local function GetAuraList()
	local AuraList = {}
	pcall(function()
		local AuraConfig = ReplicatedStorage:WaitForChild("N1"):WaitForChild("AuraConfig")
		if AuraConfig:IsA("ModuleScript") then
			local success, data = pcall(require, AuraConfig)
			if success and type(data) == "table" then
				local auraTable = data.Auras or data
				if type(auraTable) == "table" then
					for _, aura in ipairs(auraTable) do
		...
		if #AuraList == 0 and AuraConfig:IsA("Folder") then
			local aurasFolder = AuraConfig:FindFirstChild("Auras")
			if aurasFolder and aurasFolder:IsA("Folder") then
				for _, child in ipairs(aurasFolder:GetChildren()) do
					...
					if nameAttr then
						table.insert(AuraList, tostring(nameAttr))
					else
						table.insert(AuraList, child.Name)
					end
				end
			end
		end
		if #AuraList == 0 then
			local aurasChild = AuraConfig:FindFirstChild("Auras")
			if aurasChild and aurasChild:IsA("ModuleScript") then
				local success, data = pcall(require, aurasChild)
				if success and type(data) == "table" then
...
-- 1. AUTO WINS (firetouchinterest on win pad)
local function AutoWins()
	if not HumanoidRootPart or not HumanoidRootPart.Parent then return end
	pcall(function()
		if WinPadPart and WinPadPart.Parent then
			firetouchinterest(HumanoidRootPart, WinPadPart, 0)
			task.wait(0.1)
			firetouchinterest(HumanoidRootPart, WinPadPart, 1)
		else
			CacheWinPad()
			if WinPadPart and WinPadPart.Parent then
				firetouchinterest(HumanoidRootPart, WinPadPart, 0)
...
-- 2. AUTO FARM LEVEL (AwardXp)
local function AutoLevel()
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("Progression"):WaitForChild("Remotes"):WaitForChild("AwardXp")
		Event:FireServer()
	end)
end
...
local function AutoRebirth()
	pcall(function()
		local Event = ReplicatedStorage:WaitForChild("Progression"):WaitForChild("Remotes"):WaitForChild("Rebirth")
		Event:FireServer()
	end)
end
...
local function AutoUpgrade()
	for _, upgradeName in ipairs(UpgradeList) do
		pcall(function()
			local Event = ReplicatedStorage:WaitForChild("Progression"):WaitForChild("Remotes"):WaitForChild("UpgradeRequest")
			Event:FireServer(upgradeName)
		end)
		task.wait(0.2)
	end
end

-- 5. AUTO BUY AURA (all from AuraConfig)
local function AutoBuyAura()
	...
	for _, auraName in ipairs(auraList) do
		pcall(function()
			local Event = ReplicatedStorage:WaitForChild("Progression"):WaitForChild("Remotes"):WaitForChild("AuraRequest")
			Event:FireServer(auraName)
		end)
		task.wait(0.3)
	end
end
...
-- CONFIG SAVE / LOAD
local ConfigsDir = "BedrockMiner/Configs"
local AutoloadPath = "BedrockMiner/Autoload.json"
...
pcall(function()
	...
	if ExecutorVersion == "Unknown" then
		pcall(function() if getexecutorversion then ExecutorVersion = tostring(getexecutorversion()) end end)
	end
end)
...
UpdatesBox:AddLabel({ Text = '<font color="#8a8a8a"> Last Updated 8/25/2026</font>' })
...
task.spawn(function()
	...
	if Success and Info and Info.Name then
		pcall(function() GameNameLabel:SetText('GAME - <font color="#60d888">' .. Info.Name .. '</font>') end)
	else
...
-- ===== MAIN TAB - ALL TOGGLES IN FARMING GROUP =====

local FarmBox = Tabs.Main:AddLeftGroupbox("Auto Farming", "star")

AddFeatureToggle(FarmBox, "AutoWins", {
	Text = "Auto Farm Wins", Tooltip = "Auto Farm Wins", Notify = true,
}, function(Value)
...
AddFeatureToggle(FarmBox, "AutoLevel", {
	Text = "Auto Farm Level", Tooltip = "Auto Farm Level", Notify = true,
}, function(Value)
	Config.AutoLevel = Value
	if Value then
		task.spawn(function()
			while Config.AutoLevel do
				AutoLevel()
				RunService.RenderStepped:Wait(0.000000001)
			end
		end)
	end
end)
...
AddFeatureToggle(FarmBox, "AutoRebirth", {
	Text = "Auto Rebirth", Tooltip = "Auto Rebirth", Notify = true,
}, function(Value)
...
AddFeatureToggle(FarmBox, "AutoUpgrade", {
	Text = "Auto Upgrade", Tooltip = "Auto Upgrade", Notify = true,
}, function(Value)
...
AddFeatureToggle(FarmBox, "AutoBuyAura", {
	Text = "Auto Buy Aura", Tooltip = "Auto Buy Aura", Notify = true,
}, function(Value)
...
MenuBox:AddLabel("Menu Bind"):AddKeyPicker("MenuBind", {
	...
	ChangedCallback = function(NewKey)
		if typeof(NewKey) == "EnumItem" then Config.MenuBind = NewKey.Name end
		ScheduleSave()
	end,
})
...
MenuBox:AddButton({ Text = "Stop All Features", Func = function()
	Config.AutoWins = false Config.AutoLevel = false Config.AutoRebirth = false Config.AutoUpgrade = false Config.AutoBuyAura = false
...
ConfigBox:AddButton({ Text = "Create config", Func = function()
	...
	if SaveConfigData(Name) then CurrentConfig = Name RefreshConfigList(Name) Notify("Config", "Config '" .. Name .. "' created", "Success")
...
ConfigBox:AddButton({ Text = "Load config", Func = function()
	...
	if LoadConfig(Name, false) then Notify("Config", "Config '" .. Name .. "' loaded", "Success") end
end, Tooltip = "Load config" })

ConfigBox:AddButton({ Text = "Overwrite config", Func = function()
	local Name = CurrentConfig if not Name then Notify("Config", "Select a config first", "Warning") return end
	if SaveConfigData(Name) then Notify("Config", "Config '" .. Name .. "' overwritten", "Success") else Notify("Config", "Config saving not supported", "Error") end
...
ConfigBox:AddButton({ Text = "Delete config", Func = function()
	...
	CurrentConfig = nil RefreshConfigList() UpdateAutoloadLabel() Notify("Config", "Config '" .. Name .. "' deleted", "Warning")
...
ConfigBox:AddButton({ Text = "Set as autoload", Func = function()
	local Name = CurrentConfig if not Name then Notify("Config", "Select a config first", "Warning") return end
	if SetAutoload(Name) then UpdateAutoloadLabel() Notify("Config", "Autoload set to '" .. Name .. "'", "Success") end
...
task.delay(1, function()
	...
	if AutoloadName and ConfigExists(AutoloadName) then CurrentConfig = AutoloadName if LoadConfig(AutoloadName, true) then Notify("Config", "Autoloaded '" .. AutoloadName .. "'", "Success") end end
